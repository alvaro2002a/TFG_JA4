...
bool ssl_setup_key_shares(SSL_HANDSHAKE *hs, uint16_t override_group_id) {
  ...
  const uint8_t limit =
      (key_shares_limit >= 1 && key_shares_limit <= 3) ? key_shares_limit : 2;
  ...
  if (override_group_id == 0) {
    ...
    for (size_t i = 1; i < groups.size(); i++) {
      if (enable_second_key_share && second_group_id == 0 &&
          (is_custom || (is_post_quantum_group(group_id) !=
                         is_post_quantum_group(groups[i])))) {
...
//
// Generate the extension_permutation array from a customized extension order
// string.
//
